...
public class EmailGeneratorService {
	...
	private String extractGeminiResponse(String responseText) {
		try {
			...
			log.error("[EmailGeneratorService] [generateEmailContent] Error...");
			e.printStackTrace();
		}
		return null;
	}
	...
	private String buildPrompt(EmailRequestDto emailRequestDto) {
		...
		if (StringUtils.isNotEmpty(emailRequestDto.getTone())) {
			Map<String, String> toneInstructions = Map.of("Professional",
					"Use a concise, respectful, and formal tone suitable for workplace communication. ", "Friendly",
					"Use a warm, conversational, and casual tone, like speaking to a colleague. ", "Apologetic",
					"Use a polite and understanding tone that sincerely conveys an apology. ", "Formal",
					"Use structured, business-like language with no contractions. ", "Casual",
					"Use relaxed and informal language as if chatting informally. ");
	...
	private String callGemini(String prompt) {
		try {
			...
			log.error("[EmailGeneratorService] [callGemini] Error...");
			e.printStackTrace();
